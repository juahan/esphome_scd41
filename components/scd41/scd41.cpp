...
static const char *const TAG = "scd41";

static const uint16_t SCD41_CMD_GET_FIRMWARE_VERSION = 0xd100;
static const uint16_t SCD41_CMD_START_CONTINUOUS_MEASUREMENTS = 0x0010;
static const uint16_t SCD41_CMD_ALTITUDE_COMPENSATION = 0x5102;
static const uint16_t SCD41_CMD_AUTOMATIC_SELF_CALIBRATION = 0x5306;
static const uint16_t SCD41_CMD_GET_DATA_READY_STATUS = 0x0202;
static const uint16_t SCD41_CMD_READ_MEASUREMENT = 0x0300;

/// Commands for future use
static const uint16_t SCD41_CMD_STOP_MEASUREMENTS = 0x0104;
static const uint16_t SCD41_CMD_MEASUREMENT_INTERVAL = 0x4600;
static const uint16_t SCD41_CMD_FORCED_CALIBRATION = 0x5204;
static const uint16_t SCD41_CMD_TEMPERATURE_OFFSET = 0x5403;
static const uint16_t SCD41_CMD_SOFT_RESET = 0xD304;

void SCD41Component::setup() {
  ESP_LOGCONFIG(TAG, "Setting up scd41...");
  ...
  if (!this->write_command_(SCD41_CMD_GET_FIRMWARE_VERSION)) {
  ...
  ESP_LOGD(TAG, "SCD41 Firmware v%0d.%02d", (uint16_t(raw_firmware_version[0]) >> 8),
           uint16_t(raw_firmware_version[0] & 0xFF));

  if (this->temperature_offset_ != 0) {
    if (!this->write_command_(SCD41_CMD_TEMPERATURE_OFFSET, (uint16_t)(temperature_offset_ * 100.0))) {
      ESP_LOGE(TAG, "Sensor SCD41 error setting temperature offset.");
  ...
  if (this->altitude_compensation_ != 0xFFFF) {
    if (!this->write_command_(SCD41_CMD_ALTITUDE_COMPENSATION, altitude_compensation_)) {
      ESP_LOGE(TAG, "Sensor SCD41 error setting altitude compensation.");
      this->error_code_ = MEASUREMENT_INIT_FAILED;
      this->mark_failed();
      return;
    }
  ...
  if (!this->write_command_(SCD41_CMD_AUTOMATIC_SELF_CALIBRATION, enable_asc_ ? 1 : 0)) {
    ESP_LOGE(TAG, "Sensor SCD41 error setting automatic self calibration.");
    this->error_code_ = MEASUREMENT_INIT_FAILED;
    this->mark_failed();
    return;
  }
  ...
  if (!this->write_command_(SCD41_CMD_START_CONTINUOUS_MEASUREMENTS, this->ambient_pressure_compensation_)) {
    ESP_LOGE(TAG, "Sensor SCD41 error starting continuous measurements.");
    this->error_code_ = MEASUREMENT_INIT_FAILED;
    this->mark_failed();
    return;
  }
...
void SCD41Component::dump_config() {
  ESP_LOGCONFIG(TAG, "scd41:");
  LOG_I2C_DEVICE(this);
  if (this->is_failed()) {
...
void SCD41Component::update() {
  ...
  if (!this->write_command_(SCD41_CMD_GET_DATA_READY_STATUS)) {
  ...
  if (!this->write_command_(SCD41_CMD_READ_MEASUREMENT)) {
...
bool SCD41Component::write_command_(uint16_t command) {
  // Warning ugly, trick the I2Ccomponent base by setting register to the first 8 bit.
  return this->write_byte(command >> 8, command & 0xFF);
}

bool SCD41Component::write_command_(uint16_t command, uint16_t data) {
  uint8_t raw[5];
  raw[0] = command >> 8;
  raw[1] = command & 0xFF;
  raw[2] = data >> 8;
...
uint8_t SCD41Component::sht_crc_(uint8_t data1, uint8_t data2) {
  uint8_t bit;
  uint8_t crc = 0xFF;
...
bool SCD41Component::read_data_(uint16_t *data, uint8_t len) {
